...
- `deposit.timeout` — время подтверждения истекло, заказ возвращён в биржу (performer + customer)
...
- ✅ Фоновый worker `deposit-deadline-timeout`, который:
  - ✅ Запускается каждые 5 минут через scheduler
  - ✅ Находит заказы со статусом `requires_confirmation` и `depositDeadline < now`
  - ✅ Переводит заказ в `published` (возвращает в биржу)
  - ✅ Освобождает escrow исполнителя (статус `released`)
  - ✅ Удаляет `order_matches` и создаёт новые для биржи
  - ✅ Отправляет Push исполнителю: "Гарантийная сумма возвращена"
  - ✅ Отправляет Push заказчику: "Заказ не подтверждён, возвращён в биржу"
  - ✅ Пишет `order_status_events` ("Час підтвердження заказчиком минув")
  - ✅ Отправляет WebSocket `deposit.timeout` исполнителю
