...
exports.startExpiredOrdersWorker = startExpiredOrdersWorker;
exports.scheduleExpiredOrderChecks = scheduleExpiredOrderChecks;
...
const jobs_1 = require("../queues/jobs");
/**
 ...
 */
function startExpiredOrdersWorker() {
    const expo = new expoPush_service_1.ExpoPushService(prisma_1.prisma);
    return new bullmq_1.Worker("expired-orders", async (job) => {
        ...
        if (!order) {
            process.stdout.write(JSON.stringify({ level: "info", msg: "expired_order_skipped", reason: "order_not_found", orderId }) + "\n");
            return;
        }
        ...
        if (!order.depositDeadline) {
            process.stdout.write(JSON.stringify({ level: "info", msg: "expired_order_skipped", reason: "no_deadline", orderId }) + "\n");
            return;
        }
        const now = new Date();
        ...
        await (0, jobs_1.enqueueMatchNewOrder)(orderId);
        process.stdout.write(JSON.stringify({
            level: "info",
 ...
 */
async function scheduleExpiredOrderChecks() {
    const now = new Date();
